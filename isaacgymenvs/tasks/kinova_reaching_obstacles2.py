...
class KinovaReachingObstacles2(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render) -> None:
        ...
        # dimensions
        # obs include: 'joint_pos': {"pos" (6), "eef_pos_to_target" (3), "eef_pos_to_obs_one" (3), "eef_pos_to_obs_two" (3), "eef_pos_to_obs_three" (3)}
    ...
    def _create_envs(self) -> None:
        ...
        # load obstacles
        obstacle_size = [0.05, 0.05, 1.0]
        self.obstacle_size = to_torch(obstacle_size, device=self.device, dtype=torch.float).repeat(self.num_envs, 1)
        obs_asset_options = gymapi.AssetOptions()
        obs_asset_options.fix_base_link = True
        obs_asset = self.gym.create_box(self.sim, *obstacle_size, obs_asset_options)
        self.obs_default_pos = gymapi.Vec3(0.0, 0.0, table_thickness+obstacle_size[-1]/2)
        self.obs_default_quat = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
        obs_pose = gymapi.Transform()
        obs_pose.p = self.obs_default_pos
        obs_pose.r = self.obs_default_quat
        ...
        max_agg_bodies = num_kinova_bodies + 4     # 1 for table, 3 for obstacles
        max_agg_shapes = num_kinova_shapes + 4     # 1 for table, 3 for obstacles
        ...
        for i in range(self.num_envs):
            ...
            # add obstacles
            self.gym.create_actor(env, obs_asset, obs_pose, "obstacle_one", i, 1, 0)
            self.gym.create_actor(env, obs_asset, obs_pose, "obstacle_two", i, 1, 0)
            self.gym.create_actor(env, obs_asset, obs_pose, "obstacle_three", i, 1, 0)
    ...
    def init_data(self):
        env_ptr = self.envs[0]
        self.table_handle = 0
        self.obs_handle_one = 2
        self.obs_handle_two = 3
        self.obs_handle_three = 4
        ...
        self._obs_state_one = self._root_tensor[:, self.obs_handle_one, :]
        self._obs_state_two = self._root_tensor[:, self.obs_handle_two, :]
        self._obs_state_three = self._root_tensor[:, self.obs_handle_three, :]

        # Initialize target and obstacles position (num_envs, pos)
        self.target_pos = torch.zeros(self.num_envs, 3, device=self.device, dtype=torch.float32)
        self.obs_pos_one = torch.zeros_like(self.target_pos)
        self.obs_pos_two = torch.zeros_like(self.target_pos)
        self.obs_pos_three = torch.zeros_like(self.target_pos)
        ...
        # Initialize indices (globle, each envs contains 5 actors (table 3*obstacles arm))
    ...
    def _reset_obstacles_postion(self, env_ids, target_theta, target_radius):
        # obstacle one same theta, x/2, y/2
        self.obs_pos_one = self.target_pos.clone()
        self.obs_pos_one[env_ids, 0] /= 2
        self.obs_pos_one[env_ids, 1] /= 2
        self.obs_pos_one[env_ids, 2] = self.obs_default_pos.z

        # obstacle two theta - pi/12, radius * 0.75
        obs_theta_two = target_theta - (torch.pi / 12)
        obs_radius_two = target_radius * 0.75
        obs_x_two = obs_radius_two * torch.cos(obs_theta_two)
        obs_y_two = obs_radius_two * torch.sin(obs_theta_two)
        self.obs_pos_two[env_ids, 0] = obs_x_two[:, 0]
        self.obs_pos_two[env_ids, 1] = obs_y_two[:, 0]
        self.obs_pos_two[env_ids, 2] = self.obs_default_pos.z

        # obstacle three theta + pi/6, radius * 0.9
        obs_theta_three = target_theta + (torch.pi / 6)
        obs_radius_three = target_radius * 0.9
        obs_x_three = obs_radius_three * torch.cos(obs_theta_three)
        obs_y_three = obs_radius_three * torch.sin(obs_theta_three)
        self.obs_pos_three[env_ids, 0] = obs_x_three[:, 0]
        self.obs_pos_three[env_ids, 1] = obs_y_three[:, 0]
        self.obs_pos_three[env_ids, 2] = self.obs_default_pos.z

        # Target: tensor([0.5813, 0.1484, 0.6000], device='cuda:0', dtype=torch.float32)
        # Obstacle 1: tensor([0.2907, 0.0742, 0.5200], device='cuda:0', dtype=torch.float32)
        # Obstacle 2: tensor([ 0.4500, -0.0053,  0.5200], device='cuda:0', dtype=torch.float32)
        # Obstacle 3: tensor([0.3863, 0.3773, 0.5200], device='cuda:0', dtype=torch.float32)
        # print(f"Target: {self.target_pos[env_ids[0]]}, Obstacle 1: {self.obs_pos_one[env_ids[0]]}, Obstacle 2: {self.obs_pos_two[env_ids[0]]}, Obstacle 3: {self.obs_pos_three[env_ids[0]]}")

        self._obs_state_one[env_ids, :3] = self.obs_pos_one
        self._obs_state_one[env_ids, 6] = 1.0
        self._obs_state_two[env_ids, :3] = self.obs_pos_two
        self._obs_state_two[env_ids, 6] = 1.0
        self._obs_state_three[env_ids, :3] = self.obs_pos_three
        self._obs_state_three[env_ids, 6] = 1.0

        obs_ids = self._global_indices[env_ids, 2:].flatten()
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self._root_tensor),
            gymtorch.unwrap_tensor(obs_ids), len(obs_ids))
    ...
    def _reset_target_position(self, env_ids, random=True):
        ...
        self._reset_obstacles_postion(env_ids, theta, radius)
    ...
    def _update_states(self):
        self.states.update({
            ...
            "obs_pos_one": self._obs_state_one[:, :3],
            "obs_pos_two": self._obs_state_two[:, :3],
            "obs_pos_three": self._obs_state_three[:, :3],
            # Targets
            "target_pos": self.target_pos,
            "eef_pos_to_target": self._eef_state[:, :3] - self.target_pos,
            "eef_pos_to_obs_one": self._eef_state[:, :3] - self._obs_state_one[:, :3],
            "eef_pos_to_obs_two": self._eef_state[:, :3] - self._obs_state_two[:, :3],
            "eef_pos_to_obs_three": self._eef_state[:, :3] - self._obs_state_three[:, :3],
        })
        # print("pos:", self._pos[0])
        # print("vel:", self._vel[0])
        # print("eef_pos:", self._eef_state[0, :3])
        # print("eef_quat:", self._eef_state[0, 3:7])
        # print("obs_pos_one:", self._obs_state_one[0, :3])
        # print("obs_pos_two:", self._obs_state_two[0, :3])
        # print("obs_pos_three:", self._obs_state_three[0, :3])
    ...
    def compute_observations(self):
        ...
        obs = ["pos", "eef_pos_to_target", "eef_pos_to_obs_one", "eef_pos_to_obs_two", "eef_pos_to_obs_three"]
    ...
    def simulate(self) -> None:
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            # self.reset_idx(torch.tensor([0], device=self.device, dtype=torch.int))
            # for i in range(300):
                # Step the physics

            self.step(torch.tensor([[-1, 0.5, 0.2, 0.2, 0.0, 0.0]], device=self.device, dtype=torch.float))
