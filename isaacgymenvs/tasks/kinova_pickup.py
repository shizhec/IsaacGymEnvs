...
class KinovaPickup(VecTask):
    ...
    def _choose_target(self, env_ids):
        ...
        sampled_object_idx = torch.ones((num_resets,), device=self.device, dtype=torch.long) * 2
    ...
    def pre_physics_step(self, actions: torch.Tensor):
        ...
        u_fingers[:, :] = torch.where(u_gripper >= 0.0, self.kinova_upper_limits_pos[self.n_dofs_arm:],
                                      self.kinova_lower_limits_pos[self.n_dofs_arm:])
        self._pos_control[:, self.n_dofs_arm:] = self._pos_control[:, self.n_dofs_arm:] + self.dt * u_fingers * self.action_scale_gripper
...
def compute_kinova_reward(reset_buf, progress_buf, states, reward_settings, max_episode_length):
    ...
    print(f"eef_target_dist_reward: {eef_target_dist_reward}")
    reward = eef_target_dist_reward
    ...
    print(f"grasping_reward: {grasping_reward}")
    ...
    lift_reward = reached * (1 - torch.tanh(10.0 * lift_distance)) * reward_settings["lift_scale"]
    print(f"lift_reward: {lift_reward}")
    reward += lift_reward
